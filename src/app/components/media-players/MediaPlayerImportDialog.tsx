/**
 * Media Player Import Pane
 *
 * Slide-in side panel for importing media players from Google Sheets CSV/TSV files.
 */
...
import { importMediaPlayersFromGoogleSheets, downloadMediaPlayersTemplate } from '../../utils/mediaPlayersImport';
import { ImportResult } from '../../utils/sheetsImport';
import { MediaPlayer } from '../../types';

interface MediaPlayerImportDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImport: (players: Partial<MediaPlayer>[]) => Promise<void>;
}

export function MediaPlayerImportDialog({ open, onOpenChange, onImport }: MediaPlayerImportDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const importResult = importMediaPlayersFromGoogleSheets(text, delimiter);
      setResult(importResult);
      if (importResult.success && importResult.data && importResult.data.length > 0) {
        await onImport(importResult.data as Partial<MediaPlayer>[]);
      }
    } catch (error) {
    ...
    <Sheet open={open} onOpenChange={handleClose}>
      <SheetContent side="right" className="sm:max-w-lg w-full p-0 gap-0 flex flex-col">
        <SheetHeader className="px-6 py-5 border-b flex-shrink-0">
          <SheetTitle className="flex items-center gap-2"><FileSpreadsheet className="size-5" />Import Media Players</SheetTitle>
          <SheetDescription>Import media player data from a Google Sheets export (CSV or TSV format)</SheetDescription>
        </SheetHeader>
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => downloadMediaPlayersTemplate()} className="gap-2"><Download className="size-4" />Template</Button>
          </div>
          <div className="space-y-2">
            <Label htmlFor="mp-file-upload">Select File</Label>
            <input ref={fileInputRef} id="mp-file-upload" type="file" accept=".csv,.tsv,.txt" onChange={handleFileChange} className="flex-1 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-slate-900 file:text-white hover:file:bg-slate-800" />
